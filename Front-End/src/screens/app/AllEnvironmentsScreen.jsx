...
function AllEnvironmentsScreen() {
  ...
  const suggestions = ENVIRONMENTS.map((env) => t(env.nameKey))
        ...
        {isEmpty && hasActiveFilter ? (
          <div className="no-results">
            ...
            <p className="no-results-title">
              {t('Not Founded') || 'No se encontraron resultados'}
            </p>
            <p className="no-results-sub">
              {t('Retry Search') || 'Intenta con otro término de búsqueda'}
            </p>
            <button className="no-results-clear" onClick={() => setFilters({ name: '', co2: '', db: '', temp: '' })}>
              {t('Another search') || 'Limpiar filtros'}
            </button>
          </div>
        ) : (
          <>
