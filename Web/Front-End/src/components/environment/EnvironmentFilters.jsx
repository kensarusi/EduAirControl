...
// ── Rangos predefinidos por métrica ────────────────────────────
const RANGES = {
  co2: [
...
// ── FloatingInput con sugerencias (nombre) ──────────────────────
...
function RangeDropdownInput({ label, rangeKey, selectedRange, onSelect }) {
  ...
  const ICON = { co2: '🌫️', db: '🔊', temp: '🌡️', noise: '🔉' }

  return (
    <div
      className={`floating-field ${hasValue ? 'has-value' : ''}`}
      ref={wrapperRef}
  ...
  return (
    <div className="all-env-headers">
      ...
      <RangeDropdownInput
        label="dB"
        rangeKey="db"
        selectedRange={filters.db}
        onSelect={handleRange('db')}
      />
      <RangeDropdownInput
        label="Temp"
        rangeKey="temp"
